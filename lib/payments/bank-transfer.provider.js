...
export const bankTransferProvider = {
  ...
  async getBankDetailsAsync() {
    const config = await getBankConfig();
    const fallback = this.getBankDetails();
    return {
      accountTitle: config.accountTitle || fallback.accountTitle,
      accountNumber: config.accountNumber || fallback.accountNumber,
      bankName: config.bankName || fallback.bankName,
      branch: config.branch || fallback.branch,
    };
  },
